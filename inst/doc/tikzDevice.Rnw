...
<<xelatexFontVariantExample,keep.source=TRUE,echo=TRUE,eval=FALSE,results=hide>>=
...
\begin{figure}[!h]
\centering
\includegraphics{figs/xelatexEx.pdf}
...
% -----------------------------------------------------------------------------
\chapter{The \pkgopt{getLatexCharMetrics} and \pkgopt{getLatexStrWidth} Functions}
...
\begin{flexlabelled}{code}{*}{0.5em}{0.5em}{*}{\leftmargin}
  ...
  \item[face]
    An integer specifying the \pkg{R} font face to use during metric
...
TeX Users Group and a new version is released every year. Note that the version
of TeX Live provided by many Linux package management systems is the 2007
version. We recommend using TeX Live 2008 or higher as the \code{tlmgr} package
...
for TeX Live 2008 this amounts to approximately 1.2 GB. This is quite a large
...
a TDS-complient configuration. TDS-complient archives usually have the phrase
...
For packages that aren't provided in TDS-complient form look for installation
...
\begin{tikzpicture}
  ...
  \node[text width = 4.3in] (r code) {
    \begin{tikzCodeNode}[title={R User Types}, listing style=sweavechunk]
tikz('Rplot.tex')
plot(1)
dev.off()
    \end{tikzCodeNode}
