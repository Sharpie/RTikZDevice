...
<<setup,echo=F,results=hide>>=
	...
	# Ok... I've had it with some of Sweave's design
	# choices. This is it. I'm declaring war.
	
	# Begin Mother of All R Hacks
	
	# We load a new Sweave function that has had some
	# sanity beaten into it's output routines.
	source('castrateSweave.R')
	
	# Then, we find out where the bastard lives.
	env <- as.environment('package:utils')
	
	# Then we break into it's house.
	...
	assign('makeRweaveLatexCodeRunner',patchedCodeRunner,envir=env)
	lockBinding('makeRweaveLatexCodeRunner',env)
	
	#  ...we take something with us when we leave...
	...
	# ...and beat it over the head with some common sense.
...
\newsavebox{\tikzCodeBody}

%\lstset{language=R,
%	moredelim=[is][\color{gray}]{\\begin\{Soutput\}}{\\end\{Soutput\}}
%}
%\begin{lrbox}{\tikzCodeBody}
%\begin{lstlisting}
%# Get metrics for 'y'
%getLatexCharMetrics(121)
