...
\subsubsection{An Xe\LaTeX{} example}

It is also possible to use Xe\LaTeX{} with \pkg{tikzDevice} that introduces some interesting possibilities.  The following example was inspired by Dario Taraborelli and his article \textit{The Beauty of LaTeX}.
...
options( tikzLatexPackages = c(
  ...
  ,"\\newcommand{\\redf}[1]
  	{\\color{myred}\\fontspec[Variant=7]{Zapfino}#1}"
  ,"\\newcommand{\\redg}[1]
