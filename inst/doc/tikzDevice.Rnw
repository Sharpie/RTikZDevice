...
\Sexpr{print(pdfLink)}Note that files produced using the \code{standAlone} option cannot be included in \LaTeX documents using \verb`\input{}`! Use \verb`\includegraphics{}` or load the \code{pdfpages} package and use \verb`\includepdf{}`.
