...
{A lot of power is given to you through these global options, and with great power comes great responsibility.  For example, if you do not include the \TikZ{} package in the \code{tikzLatexPackages} option then all of the string metric calculations will fail.  There are innumerable ways for packages to clash in \LaTeX{} so just be aware. 
}
...
It is also possible to use Xe\LaTeX{} with \pkg{tikzDevice} that introduces some interesting possibilities.  The following example was taken inspired by Dario Taraborelli and his article \textit{The Beauty of LaTeX}.
...
Compiling the resulting file \code{xelatexEx.tex} with Xe\LaTeX{} will produce the output below!
