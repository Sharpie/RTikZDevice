...
\section{R Options That Affect Package Behavior}
  \label{sec:options}
...
\subsection{A Word of Caution About Setting Options}
...
\subsection{\code{tikzDefaultEngine}}

This option specifies which typesetting engine functions in the
\pkg{tikzDevice} package will prefer. Current possible values are
\code{pdftex}, \code{xetex} or \code{luatex} which will respectively trigger
the use of the \hologo{pdfLaTeX}, \hologo{XeLaTeX} or \hologo{LuaLaTeX}
compilers.
...
\subsection{\code{tikzLatex}, \code{tikzXelatex} and \code{tikzLualatex}}
...
\subsection{\code{tikzMetricsDictionary}}
...
\subsection{\code{tikzDocumentDeclaration}}
...
\subsection{\code{tikzLatexPackages}, \code{tikzXelatexPackages} and \code{tikzLualatexPackages}}
...
\subsection{\code{tikzMetricPackages} and \code{tikzUnicodeMetricPackages}}
...
\subsection{\code{tikzSanitizeCharacters} and \code{tikzReplacementCharacters}}
...
\subsection{\code{tikzRasterResolution}}
...
\subsection{\code{tikzPdftexWarnUTF}}
...
\subsection{Getting around special \LaTeX{} characters}
...
\subsection{\code{tikz()} vs. \code{pdf()} for \code{plotmath} symbols and Unicode characters}
