...
<<setup,echo=F,results=hide>>=
	...
	#                      Sweave
	#
	#   chunktitle:  A straight TeX string that will be used to title the code chunk
	#                     if present. Do not escape backslashes or the output will look
	#                     funny. Also, there are a few characters you cannot use with this
	#                     option, namely '=' and ',' as Sweave uses them to separate other
	#                     options in the code chunk option line.
	#
	# chunkfooter: Same as chunktitle except the message appears centered at the
	#                      bottom of the code block. Both chunktitle and chunkfooter must
	#                      be set in the code chunk header and should probably be set last.
...
<<simpleEx,echo=T,results=hide,chunktitle=Simple usage of \code{tikz()}>>=
require(tikzDevice)
...
<<latexEx,echo=T,results=hide,keep.source=T,chunktitle=Using \LaTeX\ in plots>>=
require(tikzDevice)
tikz('latexEx.tex',
...
<<bareBonesExample,echo=T,results=hide,keep.source=T,chunktitle=Preparing raw output using \code{bareBones}>>=
require(tikzDevice)
...
<<standAloneExample,echo=T,results=hide,keep.source=T,chunktitle=Generation of self contained output>>=
require(tikzDevice)
...
<<strWidthDemo,echo=T,chunktitle=How to calculate string widths with \code{getLatexStringWidth()}>>=
...
<<charMetricDemo,echo=T,keep.source=T,chunktitle=How to calculate character metrics>>=
# Get metrics for 'y'
...
<<charMetricErrors,echo=T,keep.source=T,chunktitle=How {\bfseries\color{red}not} to calculate character metrics>>=
