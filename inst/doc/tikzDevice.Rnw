...
A number of global options are provided by \pkg{tikzDevice}.  These options allow for the use of custom \code{documentclass} declarations, \LaTeX{} packages, and typesetting engines (e.g. XeLaTeX). The global options are:
\begin{description}
\item[\code{tikzDocumentDeclaration}]{ The documentclass declaration}
\begin{tikzDocExample}[title={Default}]
options( tikzDocumentDeclaration = "\\documentclass{article}" )
\end{tikzDocExample}

\item[\code{tikzFooter}]{ The footer to be used only when \code{standAlone==TRUE} }
\begin{tikzDocExample}[title={Default}]
options( tikzFooterDefault = c( "\\end{document}" ) )
\end{tikzDocExample}

\item[\code{tikzLatex}]{  }
The default is searched for when the package is loaded, otherwise it can be set manually. 

\item[\code{tikzLatexPackages}]{ These are the packages which are included when using the \code{standAlone} option as well as when font metric are calculated. }
\begin{tikzDocExample}[title={Default}]
...
\end{tikzDocExample}
\item[\code{tikzMetricPackages}]{ A character vector. These are the packages which are additionally loaded when doing font metric calculations }
\begin{tikzDocExample}[title={Default}]
...
\item[\code{tikzMetricsDictionary}]{ The path to the global \TikZ{} font metric dictionary, we recommend setting this your \code{.Rprofile} file }
\end{description}

All of the options can be set by using \code{options(<option> = <value>)}.
